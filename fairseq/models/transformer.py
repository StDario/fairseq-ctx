...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths, only_emb=False, emb_with_pos=False):
        """
        Args:
        ...
        if only_emb and not emb_with_pos:
             return x
        if self.embed_positions is not None:
            x += self.embed_positions(src_tokens)
        if only_emb and emb_with_pos:
            return x
...
class TransformerDecoderLayer(nn.Module):
    ...
    `dropout -> add residual`. We default to the approach in the paper, but the
    tensor2tensor approach can be enabled by setting
    *args.decoder_normalize_before* to ``True``.ear(in_features=256, out_featur
    ...
    def __init__(self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False, enc_kv_dim=None):
        super().__init__()
        ...
        if no_encoder_attn:
            ...
            self.encoder_attn = MultiheadAttention(
                ...
                kdim=enc_kv_dim, vdim=enc_kv_dim
            )
