...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, dummy_batch=None, oom_batch=None):
        ...
        self._reconstruct_optimizer = None
        self._prev_grad_norm = None
        self._wrapped_model = None

        reconstruct_param_names = ['doc_encoder.compression_network.embed_sentence_positions.weight',
                                   'doc_encoder.compression_network.compression_embed.weight',
                                   'doc_encoder.compression_network.attn.in_proj_weight',
                                   'doc_encoder.compression_network.attn.in_proj_bias',
                                   'doc_encoder.compression_network.attn.out_proj.weight',
                                   'doc_encoder.compression_network.attn.out_proj.bias',
                                   'doc_encoder.compression_network.layer_norm.weight',
                                   'doc_encoder.compression_network.layer_norm.bias']
        recon_module_param_names = ["module." + pn for pn in reconstruct_param_names]
        self.reconstruct_parameter_names = set(recon_module_param_names + reconstruct_param_names)
    ...
    @property
    def reconstruct_optimizer(self):

        if self.args.reconstruct_compressed:
            if self._reconstruct_optimizer is None:
                self._build_reconstruct_optimizer()
            return self._reconstruct_optimizer
        return None
    ...
    def _build_reconstruct_optimizer(self):
        params = [v for k, v in self.model.named_parameters() if
                  v.requires_grad and k in self.reconstruct_parameter_names]

        if self.args.fp16:
            self._reconstruct_optimizer = optim.FP16Optimizer.build_optimizer(self.args, params)
        else:
            self._reconstruct_optimizer = optim.build_optimizer(self.args, params)

    def _build_optimizer(self):
        # params = list(filter(lambda p: p.requires_grad, self.model.parameters()))
        params = [v for k, v in self.model.named_parameters() if v.requires_grad and k not in self.reconstruct_parameter_names]
    ...
    def load_checkpoint(
        ...
        if last_optim_state is not None and not reset_optimizer:
            ...
            if 'amsgrad' not in last_optim_state['param_groups'][0]:
                last_optim_state['param_groups'][0]['amsgrad'] = False

            if 'max_grad_norm' not in last_optim_state['param_groups'][0]:
                last_optim_state['param_groups'][0]['max_grad_norm'] = 0

            if 'bias_correction' not in last_optim_state['param_groups'][0]:
                last_optim_state['param_groups'][0]['bias_correction'] = 0
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                ...
                loss, sample_size, logging_output = self.task.train_step(
                    ...
                    ignore_grad, self.reconstruct_optimizer
                )
        ...
        try:
            ...
            if sample_size > 0:
                ...
                if self.args.reconstruct_compressed:
                    self.reconstruct_optimizer.multiply_grads(self.args.distributed_world_size / float(sample_size))

            # clip grads
            grad_norm = self.optimizer.clip_grad_norm(self.args.clip_norm)
            if self.args.reconstruct_compressed:
                reconstruct_grad_norm = self.reconstruct_optimizer.clip_grad_norm(self.args.clip_norm)

            self._prev_grad_norm = grad_norm

            # take an optimization step
            if self.args.reconstruct_compressed:
                self.reconstruct_optimizer.step()
            self.optimizer.step()
        ...
        if self.args.fp16:
            ...
            if self.args.reconstruct_compressed:
                self.meters['loss_scale'].update(self.reconstruct_optimizer.scaler.loss_scale)
    ...
    def zero_grad(self):
        ...
        if self.args.reconstruct_compressed:
            self.reconstruct_optimizer.zero_grad()
